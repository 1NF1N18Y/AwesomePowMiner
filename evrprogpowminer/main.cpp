/*
    This file is part of awesomepowminer.

    awesomepowminer is free software: you can redistribute it and/or modify
    it under the terms of the GNU General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    awesomepowminer is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU General Public License for more details.

    You should have received a copy of the GNU General Public License
    along with awesomepowminer.  If not, see <http://www.gnu.org/licenses/>.
*/

#include <CLI/CLI.hpp>

#include <awesomepowminer/buildinfo.h>
...
bool g_exitOnError = false;  // Whether or not awesomepowminer should exit on mining threads errors
...
#if ETH_DBUS
#include <awesomepowminer/DBusInt.h>
#endif
...
#undef BACKTRACE_MAX_FRAMES
    ...
    bool validateArgs(int argc, char** argv)
    {
        ...
        CLI::App app("awesomepowminer - GPU ProgPOW(0.9.4) miner for Zing");
...
//        app.add_option("-r,--reward-address", m_PoolSettings.rewardAddress, "");
    ...
    void help()
    {
        cout << "awesomepowminer - GPU ProgPOW(0.9.4) miner for Zing" << endl
             << "minimal usage : awesomepowminer [DEVICES_TYPE] [OPTIONS] -P... [-P...]" << endl
             << endl
             << "Devices type options :" << endl
             << endl
             << "    By default awesomepowminer will try to use all devices types" << endl
             ...
             << "                        awesomepowminer --help-ext con" << endl
             << endl
    ...
    void helpExt(std::string ctx)
    {
        ...
        if (ctx == "api")
        {
            cout << "API Interface Options :" << endl
                 << endl
                 << "    awesomepowminer provide an interface for monitor and or control" << endl
        ...
        if (ctx == "misc")
        {
            cout << "Miscellaneous Options :" << endl
                 ...
                 << "                        Sets the number of minutes awesomepowminer can stay" << endl
                 ...
                 << "    --exit              FLAG Stop awesomepowminer whenever an error is encountered"
                 << endl
                 << "    --ergodicity        INT[0 .. 2] Default = 0" << endl
                 << "                        Sets how awesomepowminer chooses the nonces segments to"
                 << endl
                 << "                        search on." << endl
        ...
        if (ctx == "con")
        {
            cout << "Connections specifications :" << endl
                 ...
                 << "    the fail-over disconnects awesomepowminer passes to the next connection" << endl
                 << "    available and so on till the list is exhausted. At that moment" << endl
                 << "    awesomepowminer restarts the connection cycle from the first one." << endl
                 << "    An exception to this behavior is ruled by the --failover-timeout" << endl
                 << "    command line argument. See 'awesomepowminer -H misc' for details." << endl
                 << endl
                 << "    The special notation '-P exit' stops the failover loop." << endl
                 << "    When awesomepowminer reaches this kind of connection it simply quits." << endl
                 << endl
                 << "    When using stratum mode awesomepowminer tries to auto-detect the correct" << endl
...
int main(int argc, char** argv)
{
    ...
    auto* bi = awesomepowminer_get_buildinfo();
    cout << endl
         << endl
         << "awesomepowminer " << bi->project_version << endl
    ...
    if (argc < 2)
    {
        cerr << "No arguments specified. " << endl
             << "Try 'awesomepowminer --help' to get a list of arguments." << endl
             << endl;
        return 1;
    }
    ...
    try
    {
        ...
        catch (std::invalid_argument& ex1)
        {
            cerr << "Error: " << ex1.what() << endl
                 << "Try awesomepowminer --help to get an explained list of arguments." << endl
                 << endl;
            return 1;
        }
